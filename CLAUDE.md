...
- `src/lib/components/Roulette/` — `Wheel.svelte` (pure SVG, counter-rotated labels), `WheelArea.svelte` (fullscreen takeover + confetti), `OptionsList`, `HistoryCard`, `JoinCard`, `MediaSearchInput` (debounced TMDB autocomplete), `MediaDetails` + `RulesModal` (dssoca Modal), `MediaPoster`, `IdeasEditor` (CodeMirror + yCollab CRDT), `PersonalIdeas` (debounced private notes).
...
2. **Whole-turn landing math** (`spin.svelte.ts`): rotation is cumulative; on spin it normalizes to a whole-turn boundary then adds `360·turns + align` where `align = 360 − (index+0.5)·segment` — this is what makes the wheel rest exactly on the winner. `turns` must stay an integer.
